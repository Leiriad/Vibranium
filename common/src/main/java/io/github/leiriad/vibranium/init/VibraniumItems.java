...
public class VibraniumItems {
    ...
    public static final RegistrySupplier<Item> VIBRANIUM_AXE = ITEMS.register("vibranium_axe", () -> {
        ResourceKey<Item> key = ResourceKey.create(Registries.ITEM,
                Identifier.fromNamespaceAndPath(VibraniumMod.MOD_ID, "vibranium_axe"));
        return new VibraniumAxe(VibraniumAxe.getProperties(new Item.Properties()).setId(key));
    });

    // COLORED WIRES MAP
    public static final Map<DyeColor, RegistrySupplier<Item>> COLORED_WIRES = new EnumMap<>(DyeColor.class);

    static {
        for (DyeColor color : DyeColor.values()) {
            String path = color.getName() + "_electric_wire";

            RegistrySupplier<Item> itemSupplier = ITEMS.register(path, () -> new ElectricWireItem(
                    VibraniumBlocks.ELECTRIC_WIRE.get(),
                    VibraniumBlocks.ELECTRIC_WIRE_WALL.get(),
                    color,
                    new Item.Properties()
                            .setId(ResourceKey.create(Registries.ITEM, Identifier.fromNamespaceAndPath(VibraniumMod.MOD_ID, path)))
                            .component(
                                    DataComponents.DYED_COLOR,
                                    new DyedItemColor(color.getTextureDiffuseColor())
                            )
            ));

            COLORED_WIRES.put(color, itemSupplier);
        }
    }
    ...
    public static void addItemsToTabs() {
        ...
        List.of(
                ...
                BLACK_BRICK
        ).forEach(supplier -> CreativeTabRegistry.appendStack(CreativeModeTabs.INGREDIENTS, () -> new ItemStack(supplier.get())));
        ...
        List.of(
                ...
                VIBRANIUM_HOE,
                VIBRANIUM_AXE
        ).forEach(supplier -> CreativeTabRegistry.appendStack(CreativeModeTabs.TOOLS_AND_UTILITIES, () -> new ItemStack(supplier.get())));
        ...
        COLORED_WIRES.values().forEach(supplier -> {
